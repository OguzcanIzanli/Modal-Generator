...
const Template9: React.FC<TemplateProps> = ({ modalData }) => {
  ...
  return (
    <>
      {isModalVisible && (
        <div
          className={`flex text-left relative rounded-xl w-min text-black font-sans items-center transition-transform duration-1000 ease-out ${
            id
        ...
        >
          ...
          {image?.imageUrl && (
            <div className={`${sizes}`}>
              <ImageLogo
                image={image}
                tailwindClass="w-full translate-x-[25%] rounded-lg"
              />
            </div>
          )}
